...
impl QueryPlan {
    ...
    pub fn optimize(self, context: &OptimizationContext) -> Self {
        let mut optimized_plan = self;
                ...
                match *input {
                    QueryPlan::TableScan { table_name, projection } => {
                        ...
                        if let Some(table_stats) = context.table_stats.get(&table_name) {
                            let analysis = Self::analyze_predicate_complexity(&predicate, table_stats);
                        ...
                        QueryPlan::Filter {
                            input: Box::new(QueryPlan::TableScan { table_name, projection }),
                            predicate,
                        }
                    }
                    other => {
    ...
    fn analyze_predicate_complexity(predicate: &Expression, table_stats: &TableStatistics) -> PredicateAnalysis {
            ...
            Expression::BinaryOp { left, operator, right } => {
                match operator {
                    BinaryOperator::And => {
                        ...
                        Self::extract_index_eligible_terms(left, table_stats, index_terms, range_terms, bitmap_terms, selectivity);
                        Self::extract_index_eligible_terms(right, table_stats, index_terms, range_terms, bitmap_terms, selectivity);
                                ...
                                if column_stats.is_indexed {
                                    ...
                                    if let Ok(value) = Self::literal_to_column_value(literal) {
                                        index_terms.push(IndexEligibleTerm {
                                            ...
                                            value,
                                            estimated_cardinality: if column_stats.distinct_count > 0 {
                                                table_stats.row_count / column_stats.distinct_count
                                            } else { 1 },
                                            is_primary_key: table_stats.primary_key_column.as_ref() == Some(column_name),
                                        });
                            ...
                            if let Some(column_stats) = table_stats.column_stats.get(column_name) {
                                if column_stats.is_indexed {
                                    ...
                                    if let Ok(value) = Self::literal_to_column_value(literal) {
                                        ...
                                        if let Some(existing_range) = range_terms.iter_mut().find(|r| r.column_name == *column_name) {
                                            match operator {
                                            ...
                                            let (min_val, max_val) = match operator {
                                                BinaryOperator::Less | BinaryOperator::LessEqual => (None, Some(value)),
                                                BinaryOperator::Greater | BinaryOperator::GreaterEqual => (Some(value), None),
                                                _ => (None, None),
                                            };
        ...
        match self {
            QueryPlan::NestedLoopJoin { left_input, right_input, join_type, join_condition, left_projection, right_projection } => {
                ...
                if left_tables.len() == 1 && right_tables.len() == 1 {
                    ...
                    if let Some(best_candidate) = Self::select_best_join_candidate(candidates, context) {
    ...
    fn generate_join_candidates(
        ...
        if let (Some(left_stats), Some(right_stats)) = (left_stats, right_stats) {
            ...
            if let Some(right_col_stats) = right_stats.column_stats.get(&join_condition.right_column) {
    ...
    fn create_optimized_join_plan(
        ...
        match candidate.join_method {
            ...
            JoinMethod::IndexNestedLoop { index_column } => {
                ...
                let right_table_name = Self::extract_table_names(&right_input).into_iter().next()
    ...
    fn extract_table_names(plan: &QueryPlan) -> Vec<String> {
        match plan {
            ...
            QueryPlan::NestedLoopJoin { left_input, right_input, .. } => {
                let mut tables = Self::extract_table_names(left_input);
                tables.extend(Self::extract_table_names(right_input));
                tables
            }
            QueryPlan::HashJoin { left_input, right_input, .. } => {
                let mut tables = Self::extract_table_names(left_input);
                tables.extend(Self::extract_table_names(right_input));
                tables
            }
            QueryPlan::IndexNestedLoopJoin { left_input, right_table, .. } => {
                let mut tables = Self::extract_table_names(left_input);
    ...
    fn apply_index_optimization(self, context: &OptimizationContext) -> Self {
        match self {
            QueryPlan::FilteredScan { table_name, projection, predicate } => {
                ...
                if let Some((index_column, index_value)) = Self::extract_index_predicate(&predicate, context, &table_name) {
                    QueryPlan::IndexScan {
                        table_name,
                        projection,
                        index_column,
                ...
                } else if let Some((index_column, min_val, max_val)) = Self::extract_range_predicate(&predicate, context, &table_name) {
                    QueryPlan::RangeScan {
                        table_name,
                        projection,
                        index_column,
            ...
            QueryPlan::NestedLoopJoin { left_input, right_input, join_type, join_condition, left_projection, ref right_projection } => {
                // Consider converting to IndexNestedLoopJoin if right side can use index
                if let QueryPlan::TableScan { table_name: right_table, .. } = right_input.as_ref() {
    ...
    fn apply_join_reordering(self, context: &OptimizationContext) -> Self {
        match self {
            QueryPlan::NestedLoopJoin { left_input, right_input, join_type, join_condition, left_projection, right_projection } => {
                ...
                let should_use_hash_join = context.table_stats.values()
    ...
    fn extract_index_predicate(predicate: &Expression, context: &OptimizationContext, table_name: &str) -> Option<(String, ColumnValue)> {
        match predicate {
    ...
    fn extract_range_predicate(predicate: &Expression, context: &OptimizationContext, table_name: &str) -> Option<(String, Option<ColumnValue>, Option<ColumnValue>)> {
        match predicate {
            Expression::BinaryOp { left, operator, right } => {
                    ...
                    if let Some(stats) = context.table_stats.get(table_name) {
                        if stats.indexed_columns.contains(column_name) {
                            if let Expression::Literal(literal) = right.as_ref() {
                                if let Ok(value) = Self::literal_to_column_value(literal) {
                                    match operator {
                                        BinaryOperator::Greater => return Some((column_name.clone(), Some(value), None)),
                                        BinaryOperator::GreaterEqual => return Some((column_name.clone(), Some(value), None)),
                                        BinaryOperator::Less => return Some((column_name.clone(), None, Some(value))),
                                        BinaryOperator::LessEqual => return Some((column_name.clone(), None, Some(value))),
                                        _ => {}
                                    }
                                }
                            }
    ...
    pub fn estimate_cost(&self, context: &OptimizationContext) -> CostEstimation {
        match self {
            QueryPlan::TableScan { table_name, .. } => {
                let row_count = context.table_stats.get(table_name)
            ...
            QueryPlan::FilteredScan { table_name, predicate, .. } => {
                let row_count = context.table_stats.get(table_name)
                    .map(|stats| stats.row_count)
                    .unwrap_or(1000);
            ...
            QueryPlan::NestedLoopJoin { left_input, right_input, .. } => {
                ...
                CostEstimation {
                    ...
                    estimated_cost: left_cost.estimated_cost + right_cost.estimated_cost + join_cost * 0.01,
            ...
            QueryPlan::HashJoin { left_input, right_input, .. } => {
                ...
                CostEstimation {
                    ...
                    estimated_cost: left_cost.estimated_cost + right_cost.estimated_cost + 
    ...
    fn estimate_selectivity(predicate: &Expression, context: &OptimizationContext, table_name: &str) -> f64 {
        match predicate {
                ...
                if let Expression::Identifier(column_name) = left.as_ref() {
                    if let Some(stats) = context.table_stats.get(table_name) {
                        if let Some(col_stats) = stats.column_stats.get(column_name) {
                            match operator {
                                BinaryOperator::Equal => 1.0 / col_stats.distinct_count.max(1) as f64,
