...
use crate::build_system::{BuildSystem, BuildConfig, BuildTarget, OutputType, OptimizationLevel};
...
impl CliHandler {
    pub fn new() -> Self {
        ...
        Self { build_system, package_manager }
    }
    
    pub fn handle_command(&self, command: Commands) -> Result<(), Box<dyn std::error::Error>> {
        match command {
            Commands::Build { input, output, opt_level, debug, target } => {
                self.handle_build(input, output, opt_level, debug, target)
            }
            Commands::Run { file, args } => {
                self.handle_run(file, args)
            }
            Commands::New { name, path } => {
                self.handle_new(name, path)
            }
            Commands::Install { package, version } => {
                self.handle_install(package, version)
            }
            Commands::Update => {
                self.handle_update()
            }
    ...
    fn handle_build(&self, input: Option<PathBuf>, output: Option<PathBuf>, 
                   opt_level: u8, debug: bool, target: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn compile_single_file(&self, input: &PathBuf, output: Option<PathBuf>, 
                          opt_level: u8, debug: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn handle_run(&self, file: PathBuf, args: Vec<String>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn handle_new(&self, name: String, path: Option<PathBuf>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn handle_install(&self, package: Option<String>, version: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
        match package {
            Some(pkg_name) => {
            ...
            None => {
                ...
                for (dep_name, dep_req) in &package.dependencies {
                    self.package_manager.install_package(dep_name, &dep_req.version)?;
                }
