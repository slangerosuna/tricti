use crate::ast::{SystemDef, Statement, Expression, SystemParameter, ResourceAccess, Type};
use crate::async_runtime::{AsyncExecutionError, YieldPoint, SystemExecutionResult, TaskId};
use crate::table_runtime::{TableRuntime, ColumnValue, RowId, TableError};
...
pub enum ExecutionState {
    ...
    Initialize {
        resources_to_acquire: Vec<String>,
    },
...
impl SystemStateMachineBuilder {
    ...
    fn convert_query_spec_to_execution(
        ...
        let columns = query_spec.projections
            .iter()
        ...
        Ok(QueryExecution::Select {
            columns,
            where_clause: query_spec.where_clause.as_ref().map(|expr| (**expr).clone()),
        })
    }
    ...
    fn create_async_evaluation_state(
        ...
        match expr {
            Expression::Query(query_spec) => {
                Ok(ExecutionState::ExecuteQuery {
                    table_name: query_spec.from_table.clone(),
                    query: self.convert_query_spec_to_execution(query_spec)?,
...
impl SystemStateMachineExecutor {
    ...
    pub fn execute_step(
        ...
        match current_state {
            ...
            ExecutionState::ExecuteStatements { statements, current_statement } => {
                ...
                if let ExecutionState::ExecuteStatements { current_statement, .. } = 
            ...
            ExecutionState::AwaitResource { resource_name, access_type, next_state } => {
            ...
            ExecutionState::ExecuteQuery { table_name, query, result_variable, next_state } => {
                // Execute query against table
                let result = self.execute_query(table_name, query)?;
                
                // Store result in execution context
                state_machine.execution_context.variables
            ...
            ExecutionState::AwaitSystem { system_name, task_id, next_state } => {
            ...
            ExecutionState::Sleep { duration, next_state } => {
            ...
            ExecutionState::HandleError { error, recovery_state } => {
            ...
            ExecutionState::Complete { return_value, resources_to_release } => {
                // Final cleanup
                for resource in resources_to_release {
                    state_machine.execution_context.held_resources.remove(resource);
                }
    ...
    fn execute_query(
        ...
        match query {
            QueryExecution::Select { columns, where_clause } => {
    ...
    fn evaluate_expression(
        ...
        match expr {
            Expression::Literal(lit) => {
                match lit {
                    crate::ast::Literal::Integer(int_lit) => {
