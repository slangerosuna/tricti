use peano::scheduler::*;
use peano::ast::{SystemDef, SystemParameter, ResourceAccess, Type};
...
fn test_basic_scheduler_functionality() {
    ...
    assert!(!scheduler.get_conflict_graph().has_conflict("reader1", "reader2"));
    
    // Check that readers conflict with writer
    assert!(scheduler.get_conflict_graph().has_conflict("reader1", "writer"));
    assert!(scheduler.get_conflict_graph().has_conflict("reader2", "writer"));
}
...
fn test_complex_resource_conflicts() {
    ...
    let system1 = create_test_system("multi_reader", vec![
        ("db", ResourceAccess::Immutable),
        ("cache", ResourceAccess::Immutable),
    ]);
    let system2 = create_test_system("cache_writer", vec![
        ("cache", ResourceAccess::Mutable),
    ]);
    let system3 = create_test_system("db_writer", vec![
        ("db", ResourceAccess::Mutable),
    ]);
    let system4 = create_test_system("independent", vec![
        ("file_system", ResourceAccess::Mutable),
    ]);
    ...
    // Verify conflicts
    assert!(scheduler.get_conflict_graph().has_conflict("multi_reader", "cache_writer"));
    assert!(scheduler.get_conflict_graph().has_conflict("multi_reader", "db_writer"));
    assert!(!scheduler.get_conflict_graph().has_conflict("cache_writer", "db_writer"));
    assert!(!scheduler.get_conflict_graph().has_conflict("independent", "multi_reader"));
    assert!(!scheduler.get_conflict_graph().has_conflict("independent", "cache_writer"));
    assert!(!scheduler.get_conflict_graph().has_conflict("independent", "db_writer"));
}
...
fn test_deadlock_detection() {
    ...
    let system1 = create_test_system("system1", vec![
        ("resource1", ResourceAccess::Mutable),
        ("resource2", ResourceAccess::Immutable),
    ]);
    let system2 = create_test_system("system2", vec![
    ...
    assert!(scheduler.get_conflict_graph().has_conflict("system1", "system2"));
    ...
    assert!(deadlock_result.is_ok() || matches!(deadlock_result, Err(SchedulerError::DeadlockDetected { .. })));
}
...
fn test_resource_ownership_conflicts() {
    ...
    assert!(scheduler.get_conflict_graph().has_conflict("owner", "borrower"));
    assert!(scheduler.get_conflict_graph().has_conflict("owner", "writer"));
    assert!(scheduler.get_conflict_graph().has_conflict("borrower", "writer"));
}
...
fn test_concurrent_execution_simulation() {
    ...
    let independent_system = create_test_system("independent", vec![("other_resource", ResourceAccess::Mutable)]);
...
fn test_priority_scheduling() {
    ...
    let low_priority = create_test_system("low_priority", vec![("resource1", ResourceAccess::Immutable)]);
    let high_priority = create_test_system("high_priority", vec![("resource2", ResourceAccess::Immutable)]);
    let critical_priority = create_test_system("critical_priority", vec![("resource3", ResourceAccess::Immutable)]);
...
fn test_shortest_job_first_scheduling() {
    ...
    let medium_job = create_test_system("medium_job", vec![("resource3", ResourceAccess::Immutable)]);
    
    // Add systems with different estimated runtimes
    assert!(scheduler.add_system_with_metadata(
        long_job, 
...
fn test_batch_scheduling() {
    ...
    for i in 0..5 {
        let system = create_test_system(
            &format!("system_{}", i),
            vec![(format!("resource_{}", i).as_str(), ResourceAccess::Immutable)]
        );
        assert!(scheduler.add_system(system).is_ok());
...
fn test_work_stealing_strategy() {
    ...
    let unlocker = create_test_system("unlocker", vec![("shared_resource", ResourceAccess::Mutable)]);
    ...
    let independent = create_test_system("independent", vec![("resource3", ResourceAccess::Immutable)]);
...
fn test_resource_identity_conflict_detection() {
    ...
    assert!(scheduler.get_conflict_graph().has_conflict("gui_updater", "gui_renderer"), 
    ...
    assert!(schedulable_after.is_empty(), 
...
fn test_mixed_access_patterns_same_resource_type() {
    ...
    assert!(!scheduler.get_conflict_graph().has_conflict("gui_reader1", "gui_reader2"),
    ...
    assert!(scheduler.get_conflict_graph().has_conflict("gui_reader1", "gui_writer"),
            "Immutable and mutable access to same resource type should conflict");
    assert!(scheduler.get_conflict_graph().has_conflict("gui_reader2", "gui_writer"),
            "Immutable and mutable access to same resource type should conflict");
}