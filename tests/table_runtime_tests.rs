...
mod tests {
    ...
    fn test_table_creation_without_primary_key() {
        ...
        assert!(table.is_ok(), "Tables without primary keys should be allowed");
    ...
    fn test_row_insertion_with_default_values() {
        ...
        assert_eq!(retrieved_row.values.get("display"), Some(&ColumnValue::Bool(false)));
    }
    ...
    fn test_multiple_row_insertion() {
        ...
        assert_eq!(table.find_by_primary_key(ColumnValue::U64(1)), Some(RowId(0)));
        assert_eq!(table.find_by_primary_key(ColumnValue::U64(2)), Some(RowId(1)));
        assert_eq!(table.find_by_primary_key(ColumnValue::U64(3)), Some(RowId(2)));
    }
    ...
    fn test_row_retrieval() {
        ...
        let row_id = table.insert_row(original_row.clone()).expect("Failed to insert row");
        
        let retrieved_row = table.get_row(row_id).expect("Failed to get row");
        
        assert_eq!(retrieved_row.values.get("id"), Some(&ColumnValue::U64(42)));
        assert_eq!(retrieved_row.values.get("title"), Some(&ColumnValue::String("Test App".to_string())));
        assert_eq!(retrieved_row.values.get("display"), Some(&ColumnValue::Bool(true)));
    }
    ...
    fn test_row_update() {
        ...
        updates.insert("title".to_string(), ColumnValue::String("Updated Title".to_string()));
        updates.insert("display".to_string(), ColumnValue::Bool(true));
        
        table.update_row(row_id, updates).expect("Failed to update row");
        ...
        assert_eq!(updated_row.values.get("title"), Some(&ColumnValue::String("Updated Title".to_string())));
        assert_eq!(updated_row.values.get("display"), Some(&ColumnValue::Bool(true)));
    }
    ...
    fn test_columnar_storage_access() {
        ...
        table.insert_row(create_test_row(2, "App 2", false)).unwrap();
        ...
        let title_column = table.get_column_data("title").expect("title column should exist");
        match title_column {
            ColumnData::String(values) => {
                assert_eq!(values, &vec!["App 1".to_string(), "App 2".to_string(), "App 3".to_string()]);
            }
            _ => panic!("Expected String column data"),
        }
        
        let display_column = table.get_column_data("display").expect("display column should exist");
    ...
    fn test_type_mismatch_error() {
        ...
        let mut values = HashMap::new();
        values.insert("id".to_string(), ColumnValue::String("not_a_number".to_string())); // Wrong type
        values.insert("title".to_string(), ColumnValue::String("Test".to_string()));
        ...
        match result {
            Err(TableError::TypeMismatch { column, expected, found }) => {
                assert_eq!(column, "id");
    ...
    fn test_row_id_stability_after_deletion() {
        ...
        let row_id_2 = table.insert_row(create_test_row(2, "Row 2", false)).unwrap();
        ...
        let retrieved_row_1 = table.get_row(row_id_1).expect("Row 1 should still be accessible");
        let retrieved_row_3 = table.get_row(row_id_3).expect("Row 3 should still be accessible");
        ...
        assert_eq!(table.find_by_primary_key(ColumnValue::U64(1)), Some(row_id_1));
        assert_eq!(table.find_by_primary_key(ColumnValue::U64(2)), None);
        assert_eq!(table.find_by_primary_key(ColumnValue::U64(3)), Some(row_id_3));
    }
    ...
    fn test_new_insertions_after_deletions() {
        ...
        let row_id_3 = table.insert_row(create_test_row(3, "Row 3", true)).unwrap();
        let row_id_4 = table.insert_row(create_test_row(4, "Row 4", false)).unwrap();
    ...
    fn test_update_operations_with_stable_row_ids() {
        ...
        let mut updates = HashMap::new();
        updates.insert("title".to_string(), ColumnValue::String("Updated Row 1".to_string()));
        table.update_row(row_id_1, updates).expect("Failed to update row 1");
        
        let mut updates = HashMap::new();
        updates.insert("display".to_string(), ColumnValue::Bool(false));
        table.update_row(row_id_3, updates).expect("Failed to update row 3");
        
        // Verify updates
        let updated_row_1 = table.get_row(row_id_1).expect("Failed to get updated row 1");
        let updated_row_3 = table.get_row(row_id_3).expect("Failed to get updated row 3");
        
        assert_eq!(updated_row_1.values.get("title"), Some(&ColumnValue::String("Updated Row 1".to_string())));
        assert_eq!(updated_row_3.values.get("display"), Some(&ColumnValue::Bool(false)));
        
        // Try to update deleted row - should fail
        let mut updates = HashMap::new();
        updates.insert("title".to_string(), ColumnValue::String("Should not work".to_string()));
    ...
    fn test_multiple_deletions_and_primary_key_consistency() {
        ...
        let mut row_ids = Vec::new();
        for i in 1..=5 {
            let row_id = table.insert_row(create_test_row(i, &format!("Row {}", i), i % 2 == 0)).unwrap();
            row_ids.push(row_id);
        }
        
        // Delete rows 2 and 4
        table.delete_row(row_ids[1]).expect("Failed to delete row 2");
        table.delete_row(row_ids[3]).expect("Failed to delete row 4");
        
        // Verify primary key index is consistent
        assert_eq!(table.find_by_primary_key(ColumnValue::U64(1)), Some(row_ids[0]));
        assert_eq!(table.find_by_primary_key(ColumnValue::U64(2)), None);
        assert_eq!(table.find_by_primary_key(ColumnValue::U64(3)), Some(row_ids[2]));
        assert_eq!(table.find_by_primary_key(ColumnValue::U64(4)), None);
        assert_eq!(table.find_by_primary_key(ColumnValue::U64(5)), Some(row_ids[4]));
