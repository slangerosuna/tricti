...
mod concurrency_resource_tests {
    ...
    fn test_concurrent_non_conflicting_systems() {
        ...
        assert!(schedule_result.is_ok(), "Non-conflicting systems should schedule successfully");
        ...
        assert!(scheduling_time < Duration::from_millis(100), "Scheduling should be efficient");
    ...
    fn test_immutable_borrow_contention() {
        ...
        let mutable_result = tracker.add_access("shared_resource", "writer", &ResourceAccess::Mutable);
        assert!(mutable_result.is_err(), "Mutable access should fail when immutable borrows exist");
    ...
    fn test_mutable_borrow_contention() {
        ...
        assert_eq!(tracker.get_mutable_borrower("exclusive_resource"), Some(&"writer1".to_string()));
        ...
        let result3 = tracker.add_access("exclusive_resource", "reader1", &ResourceAccess::Immutable);
        assert!(result3.is_err(), "Immutable access should fail when mutable borrow exists");
    }
    ...
    fn test_write_after_read_conflict() {
        ...
        let schedule_result = tokio_test::block_on(scheduler.schedule_systems(requests));
        assert!(schedule_result.is_ok(), "Scheduler should handle conflicts gracefully");
        
        let futures = schedule_result.unwrap();
        assert_eq!(futures.len(), 2, "Both systems should be scheduled (with ordering)");
    ...
    fn test_read_after_write_conflict() {
        ...
        let writer_result = tracker.add_access("contested_resource", "writer", &ResourceAccess::Mutable);
        assert!(writer_result.is_ok());
        
        // Reader tries to access - should fail
        let reader_result = tracker.add_access("contested_resource", "reader", &ResourceAccess::Immutable);
        ...
        let reader_retry = tracker.add_access("contested_resource", "reader", &ResourceAccess::Immutable);
        assert!(reader_retry.is_ok(), "Reader should succeed after writer releases");
    }
    ...
    fn test_complex_resource_chains() {
        ...
        assert!(tracker.add_access("resource_x", "system_a", &ResourceAccess::Immutable).is_ok());
        assert!(tracker.add_access("resource_y", "system_a", &ResourceAccess::Immutable).is_ok());
        
        // System B: needs resource Y (immutable) and Z (mutable)
        assert!(tracker.add_access("resource_y", "system_b", &ResourceAccess::Immutable).is_ok());
        assert!(tracker.add_access("resource_z", "system_b", &ResourceAccess::Mutable).is_ok());
        
        // System C: tries to get mutable access to Y - should fail
        let conflict_result = tracker.add_access("resource_y", "system_c", &ResourceAccess::Mutable);
        assert!(conflict_result.is_err(), "Should not allow mutable access to shared immutable resource");
        
        // System D: can access X (immutable) since A also has immutable access
        assert!(tracker.add_access("resource_x", "system_d", &ResourceAccess::Immutable).is_ok());
        ...
        assert!(z_conflict.is_err(), "Should not allow any access to exclusively held resource");
    ...
    fn test_resource_release_cleanup() {
        ...
        assert!(tracker.add_access("temp_resource", "system1", &ResourceAccess::Immutable).is_ok());
        assert!(tracker.add_access("temp_resource", "system2", &ResourceAccess::Immutable).is_ok());
        assert!(tracker.add_access("exclusive_resource", "system3", &ResourceAccess::Mutable).is_ok());
        
        // Verify initial state
        assert_eq!(tracker.get_immutable_borrowers("temp_resource").unwrap().len(), 2);
        assert_eq!(tracker.get_mutable_borrower("exclusive_resource"), Some(&"system3".to_string()));
        ...
        assert_eq!(tracker.get_immutable_borrowers("temp_resource").unwrap().len(), 1);
    ...
    fn test_deadlock_prevention() {
        ...
        assert!(tracker.add_access("resource_1", "system_a", &ResourceAccess::Mutable).is_ok());
        
        // System B: gets resource 2, wants resource 1 (potential deadlock)
        assert!(tracker.add_access("resource_2", "system_b", &ResourceAccess::Mutable).is_ok());
        
        // Try to create deadlock situation
        let deadlock_attempt1 = tracker.add_access("resource_2", "system_a", &ResourceAccess::Mutable);
        let deadlock_attempt2 = tracker.add_access("resource_1", "system_b", &ResourceAccess::Mutable);
        
        // Both should fail due to conflicts, preventing deadlock
        assert!(deadlock_attempt1.is_err(), "Should prevent potential deadlock");
        assert!(deadlock_attempt2.is_err(), "Should prevent potential deadlock");
        
        // Verify systems still hold their original resources
        assert_eq!(tracker.get_mutable_borrower("resource_1"), Some(&"system_a".to_string()));
        assert_eq!(tracker.get_mutable_borrower("resource_2"), Some(&"system_b".to_string()));
    }
    ...
    fn test_borrow_safety_guarantees() {
        ...
        assert!(tracker.add_access("safety_test", "reader", &ResourceAccess::Immutable).is_ok());
        let safety_violation = tracker.add_access("safety_test", "writer", &ResourceAccess::Mutable);
        assert!(safety_violation.is_err(), "Borrow safety violated: mutable after immutable");
        
        // Reset
        tracker.remove_access("safety_test", "reader", &ResourceAccess::Immutable);
        
        // Test 2: Cannot have multiple mutable borrows
        assert!(tracker.add_access("safety_test", "writer1", &ResourceAccess::Mutable).is_ok());
        let double_mutable = tracker.add_access("safety_test", "writer2", &ResourceAccess::Mutable);
        assert!(double_mutable.is_err(), "Borrow safety violated: multiple mutable borrows");
        
        // Reset
        tracker.remove_access("safety_test", "writer1", &ResourceAccess::Mutable);
        
        // Test 3: Multiple immutable borrows are OK
        assert!(tracker.add_access("safety_test", "reader1", &ResourceAccess::Immutable).is_ok());
        assert!(tracker.add_access("safety_test", "reader2", &ResourceAccess::Immutable).is_ok());
        assert!(tracker.add_access("safety_test", "reader3", &ResourceAccess::Immutable).is_ok());
        
        let borrowers = tracker.get_immutable_borrowers("safety_test").unwrap();
        assert_eq!(borrowers.len(), 3, "Multiple immutable borrows should be allowed");
    }
    ...
    fn create_system_with_resource_access(name: &str, resources: Vec<(&str, ResourceAccess)>) -> SystemDef {
        let parameters = resources
            .into_iter()
