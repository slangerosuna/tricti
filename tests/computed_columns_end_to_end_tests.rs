...
fn test_end_to_end_computed_columns() {
    ...
    let table_def = match &program.statements[0] {
        Statement::ConstDecl { value: ConstValue::TableDef(table), .. } => table.clone(),
    ...
    row_values.insert("unit_price".to_string(), ColumnValue::F64((10.0_f64).to_bits()));
    ...
    let subtotal = table.get_column_value(row_id, "subtotal").expect("Failed to get subtotal");
    ...
    let tax_amount = table.get_column_value(row_id, "tax_amount").expect("Failed to get tax_amount");
    ...
    let total = table.get_column_value(row_id, "total").expect("Failed to get total");
    ...
    let total2 = table.get_column_value(row_id, "total").expect("Failed to get total again");
...
fn test_computed_column_dependency_chain() {
    ...
    row_values.insert("tax_rate".to_string(), ColumnValue::F64((0.08_f64).to_bits()));
    
    let row = TableRow { values: row_values };
    let row_id = table.insert_row(row).expect("Failed to insert row");
    
    // Test dependency chain evaluation
    let price = table.get_column_value(row_id, "price").expect("Failed to get price");
    ...
    let tax = table.get_column_value(row_id, "tax").expect("Failed to get tax");
    ...
    let final_price = table.get_column_value(row_id, "final_price").expect("Failed to get final_price");
...
fn test_computed_column_with_string_concatenation() {
    ...
    let mut row_values = HashMap::new();
    row_values.insert("id".to_string(), ColumnValue::U64(1));
    row_values.insert("first_name".to_string(), ColumnValue::String("John".to_string()));
    row_values.insert("last_name".to_string(), ColumnValue::String("Doe".to_string()));
    
    let row = TableRow { values: row_values };
    let row_id = table.insert_row(row).expect("Failed to insert row");
    
    // Test string concatenation
    let full_name = table.get_column_value(row_id, "full_name").expect("Failed to get full_name");
...
fn test_computed_column_invalidation() {
    ...
    // Get computed value
    let total = table.get_column_value(row_id, "total_quantity").expect("Failed to get total_quantity");
